...
extension UITextField {
    ...
    func setError(_ string: String? = nil, show: Bool = true) {
        ...
        container.trailingAnchor.constraint(equalTo: superview!.trailingAnchor, constant: -10).isActive = true
        ...
        let ev = UIView()
        ev.frame =  CGRect(x: 0, y: 0, width: 30, height: 30)
        
        // Add right button to textField
        let errorButton = UIButton(type: .custom)
        errorButton.tag = 999
        errorButton.setImage(UIImage(named: "error.png"), for: .normal)
        errorButton.imageView?.contentMode = .scaleAspectFit
        errorButton.frame = CGRect(x: 0, y: 5, width: 20, height: 20)
        errorButton.addTarget(self, action: #selector(errorAction), for: .touchUpInside)
//        ev.addSubview(errorButton)
