...
class GApiResponse {
    ...
    class ReverseGio {
        ...
        class func initWithData(_ data:[String:Any]) -> ReverseGio {
            ...
            if let pId = data["place_id"] as? String,let address = data["formatted_address"] as? String {
                ...
                if ((data["thoroughfare"]) != nil && (data["thoroughfare"] as! String) != "") {
                    addr += data["thoroughfare"] as! String + ", ";
                   }
                
                if ((data["subLocality"]) != nil && (data["subLocality"] as! String) != "") {
                 addr += data["subLocality"] as! String + ", ";
                }
                
                if ((data["locality"]) != nil && (data["locality"] as! String) != "") {
                 addr += data["locality"] as! String + ", ";
                }
                
                if ((data["administrativeArea"]) != nil && (data["administrativeArea"] as! String) != "") {
                 addr += data["administrativeArea"] as! String + ", ";
                }
                
                object.formattedAddress = addr
                object.placeId = pId
            }
            return object
        }
    ...
    class Autocomplete {
        ...
        class func initWithData(_ data:[String:Any]) -> Autocomplete {
            ...
            if let pId = data["place_id"] as? String,let address = data["description"] as? String, let sf = data["structured_formatting"] as? [String:Any]{
                object.formattedAddress = address
                object.stFormatting = sf
                object.placeId = pId
            }
            return object
        }
...
class GoogleApi : NSObject {
    ...
    func getUrl(_  usedFor:UsedFor,input:String) -> String {
        ...
        case .autocomplete:
            ...
            return String(format: "%@?input=%@&key=%@&components=country:SG",GoogleUrl.autocomplete.rawValue,updatedInput,googleApiKey)
        case .placeInformation:
    ...
    func callApi(_ api : UsedFor = .autocomplete,input:GInput,completion: @escaping GCallback) {
        ...
        if let predictions = searchResultsCache[covertedInput] as? [Results], haveResults {
            ...
            for prediction in predictions {
                print("prediction 286>> ",prediction)
                ...
                if let data = data {
                    ...
                    if error != nil || isApiError {
                        ...
                        if let predictions = responseData?["predictions"] as? [Results], api == .autocomplete {
                            ...
                            for prediction in predictions {
                                print("prediction 317>> ",prediction)
                            ...
                            for prediction in predictions {
                                print("prediction 325>> ",prediction)
